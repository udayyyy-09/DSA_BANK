...
/*                      APPROACH:
We will use a map to store the sum of two elements as we have length of subarray is 2 only. we started to store sum
like this int sum = nums[i]+nums[i+1]  mp[sum]++; after inserting all pair of sum we will check for sum frequency
if frequency is >=2 we will get a two subarray because there are subarrays of len 2 whose sum are equal that's why their frequency is 2
